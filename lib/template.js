...
function commitUrl(commit, vars) {
    if (vars.gitRepo) {
        return vars.gitPlatform === 'bitbucket' ? vars.gitRepo + '/commits/' + commit : vars.gitRepo + '/commit/' + commit ;
    }
    return;
}
...
function compareUrl(oldVer, newVer, vars) {
    if (vars.gitRepo) {
        return vars.gitPlatform === 'bitbucket' ? '(' + vars.gitRepo + '/compare/commits?targetBranch=refs%2Ftags%2F' + oldVer + '&sourceBranch=refs%2Ftags%2F' + newVer + ')' : '(' + vars.gitRepo + '/compare/' + oldVer + '...' + newVer + ')';
    }
    return;
}
...
function formatCommitsToMarkdown(commits) {
    return commits.map(function (commit) {
        ...
        let formatDate = formattedDate(new Date(commit.date));
...
function prepareMarkdownTemplate(groups, vars) {
    ...
    // if we have extra comments add them before the groups
    if (vars.extraComments) {
        for (let index = 0; index < vars.extraComments.length; index++) {
            releaseComments += `\n ### ${vars.extraComments[index]} \n * list of changes \n`;
        }
    }
    ...
    return releaseHeader + releaseComments + commitGroups;
}
