...
function compareUrl(oldVer, newVer, vars) {
    if (vars.gitRepo) {
        return vars.gitRepo + '/compare/commits?targetBranch=refs%2Ftags%2F' + oldVer + '&sourceBranch=refs%2Ftags%2F' + newVer;
    }
}
...
function formatCommitsToMarkdown(commits) {
    return commits.map(function (commit) {
        ...
        return cScope + commit.convention.description + ' ([' + commit.id + ']' + '(' + commit.url + '))' + jiraTicket + author +  '\n';
    });
}
