...
let setOptionsFromProject = () => {
    ...
    if (jvmChangelog) {
        ...
        releaseVars.gitRepo = jvmChangelog.gitRepo || false;
        ...
        const fractal = require(path.resolve('./fractal-config.js'));
        releaseVars.pathToChangelog = path.resolve('./docs/CHANGELOG.md');
        releaseVars.pathToRepo = path.resolve('./.git');
        releaseVars.gitRepo = fractal ? fractal.get('project.gitrepo') : false;
    }
}
