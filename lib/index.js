...
let setOptionsFromProject = () => {
    ...
    // they should be set under jvmChangelog {}, vars: pathChangelog, pathRepo, gitRepo, gitPlatform
    // else use the defaults set here
    ...
    releaseVars.pathToChangelog = jvmChangelog && jvmChangelog.pathChangelog ? jvmChangelog.pathChangelog : path.resolve('./docs/CHANGELOG.md');
    releaseVars.pathToRepo = jvmChangelog && jvmChangelog.pathRepo ? jvmChangelog.pathRepo : path.resolve('./.git');
    releaseVars.gitRepo = jvmChangelog && jvmChangelog.gitRepo ? jvmChangelog.gitRepo : fractalRepo;
    releaseVars.gitPlatform = jvmChangelog && jvmChangelog.gitPlatform ? jvmChangelog.gitPlatform : 'bitbucket';
    releaseVars.extraComments = jvmChangelog && jvmChangelog.extraComments ? jvmChangelog.extraComments : null;
    releaseVars.ignores = jvmChangelog && jvmChangelog.ignores ? jvmChangelog.ignores : releaseVars.ignores;
    releaseVars.logNextSteps = jvmChangelog && jvmChangelog.logNextSteps ? jvmChangelog.logNextSteps : releaseVars.logNextSteps;
}
