...
let setOptionsFromProject = () => {
    ...
    console.log('setOptionsFromProject:', jvmChangelog);
    if (jvmChangelog) {
        releaseVars.pathToChangelog = jvmChangelog.pathChangelog;
        releaseVars.pathToRepo = jvmChangelog.pathRepo;
        releaseVars.gitRepo = jvmChangelog.gitRepo;
    } else {
        const projectConfig = require('../../project.config.js');
        releaseVars.pathToChangelog = path.resolve('./docs/CHANGELOG.md');
        releaseVars.pathToRepo = path.resolve('./.git');
        releaseVars.gitRepo = projectConfig.fractal.gitrepo;
    }
}
    ...
    history.on('commit', function (commit) {
        ...
        if (stopCount) {
            ...
            let commitObj = {
                ...
                url: template.commitUrl(commit.toString().substring(0, 7), releaseVars)
            }
            // here we include only commits that follow convention
            console.log('commit:', commitObj)
...
function processCommits(commits) {
    ...
    let types = commits
        ...
        // .filter(commit => commit.convention.type !== ('chore' || 'build' || 'revert' || 'test'))
        .filter(commit => {
            console.log('filter commits:', releaseVars.ignores.includes(commit.convention.type))
            !releaseVars.ignores.includes(commit.convention.type)
        })
